...
export function DataReporters({ configuration, country, users }) {
  ...
  const coordinatorsText = users.filter((u) => u.AllMemberships.includes(configuration?.DataflowCoordinatorsTag))?.map(u => u.Title).join(', ');
  ...
  const renderTitle = (params) => {
    ...
    renderReleaseDate = (params) => {
      ...
      return (
        <Box>
          {record.firstReleaseDate && <Typography style={{ display: 'inline-block' }} className="grid-text" variant="body1" component={'span'}>
            {format(record.firstReleaseDate, dateFormat)}{'(first)'}
          </Typography>}
          {record.lastReleaseDate && <Typography style={{ display: 'inline-block' }} className="grid-text" variant="body1" component={'span'}>
            {format(record.lastReleaseDate, dateFormat)}{'(latest)'}
          </Typography>}
        </Box>
      );
